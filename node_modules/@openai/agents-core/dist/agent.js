...
const agentToolInput_1 = require("./agentToolInput.js");
const agentToolRunConfig_1 = require("./agentToolRunConfig.js");
...
function getInitialModelSettingsForAgentModel(model) {
    ...
    if (typeof model === 'string' && model !== Agent.DEFAULT_MODEL_PLACEHOLDER) {
        return (0, defaultModel_1.getDefaultModelSettings)(model);
    }
    return {};
}
/**
...
class Agent extends lifecycle_1.AgentHooks {
    ...
    _modelSettingsExplicitlyConfigured;
    constructor(config) {
        super();
    ...
    get outputSchemaName() {
        ...
        else if ((0, typeGuards_1.isZodObject)(this.outputType)) {
            return 'ZodOutput';
        }
     ...
     */
    clone(config) {
        const modelSettings = 'modelSettings' in config
            ? config.modelSettings
    ...
    asTool(options) {
        const { toolName, toolDescription, customOutputExtractor, needsApproval, parameters, inputBuilder, includeInputSchema, runConfig, runOptions, resumeState, isEnabled, onStream, } = options;
        ...
        const resolvedToolName = toolName ?? (0, tools_1.toFunctionToolName)(this.name);
        const toolParameters = (parameters ??
            agentToolInput_1.AgentAsToolInputSchema);
        const hasCustomParameters = typeof parameters !== 'undefined';
        const includeSchema = includeInputSchema === true && hasCustomParameters;
        const shouldCaptureToolInput = hasCustomParameters ||
            includeSchema ||
            typeof inputBuilder === 'function';
        const schemaInfo = shouldCaptureToolInput
            ? (0, agentToolInput_1.buildStructuredInputSchemaInfo)(toolParameters, resolvedToolName, includeSchema)
            : undefined;
        const baseTool = (0, tool_1.tool)({
            ...
            execute: async (params, context, details) => {
                ...
                const runContextBase = runOptions?.context instanceof runContext_1.RunContext
                    ? runOptions.context
                    : typeof runOptions?.context !== 'undefined'
                        ? new runContext_1.RunContext(runOptions.context)
                        : context instanceof runContext_1.RunContext
                            ? context
                            : typeof context !== 'undefined'
                                ? new runContext_1.RunContext(context)
                                : new runContext_1.RunContext();
                ...
                const shouldClearToolInput = !shouldCaptureToolInput &&
                    typeof runContextBase.toolInput !== 'undefined';
                const runContext = shouldCaptureToolInput &&
                    ...
                    : shouldClearToolInput &&
                        ...
                        : runContextBase;
                const resolvedInput = await (0, agentToolInput_1.resolveAgentToolInput)({
                    params: typedParams,
                    schemaInfo,
                    inputBuilder,
                });
                ...
                const inheritedRunConfig = (0, agentToolRunConfig_1.getInheritedAgentToolRunConfig)((0, agentToolRunConfig_1.getAgentToolParentRunConfigFromDetails)(details), runConfig);
                const nestedRunConfig = (0, agentToolRunConfig_1.mergeAgentToolRunConfig)(inheritedRunConfig, runConfig);
                const runner = new run_1.Runner(nestedRunConfig);
                const resumeContextStrategy = resumeState?.contextStrategy ?? 'merge';
                const resumeContext = resumeContextStrategy === 'preferSerialized' ? undefined : runContext;
                let runInput = resolvedInput;
                if (details?.resumeState) {
                    if (resumeContextStrategy === 'preferSerialized' || !resumeContext) {
                        runInput = await runState_1.RunState.fromString(this, details.resumeState);
                    }
                    else {
                        if (resumeContextStrategy === 'merge' &&
                            context &&
                            resumeContext !== context) {
                            resumeContext._mergeApprovals(context.toJSON().approvals);
                        }
                        runInput = await runState_1.RunState.fromStringWithContext(this, details.resumeState, resumeContext, {
                            contextStrategy: resumeContextStrategy === 'replace' ? 'replace' : 'merge',
                        });
                    }
                }
                ...
                const { signal: combinedSignal, cleanup: cleanupSignalListeners } = configuredSignal && toolCallSignal
                    ? (0, abortSignals_1.combineAbortSignals)(configuredSignal, toolCallSignal)
                    : {
                ...
                try {
                    ...
                    if (completedResult.state instanceof runState_1.RunState) {
                        completedResult.state._agentToolInvocation = agentToolInvocation;
                    }
                    const completedResultWithAgentToolInvocation = completedResult;
                    ...
                    let outputText;
                    if (typeof customOutputExtractor === 'function') {
                        outputText = await customOutputExtractor(completedResultWithAgentToolInvocation);
                    }
                    else {
                        const finalOutputText = typeof completedResult.finalOutput !== 'undefined'
                        ...
                        const rawOutputText = rawResponses && rawResponses.length > 0
                            ? (0, messages_1.getOutputText)(rawResponses[rawResponses.length - 1])
                            : undefined;
                        const normalizedRawOutputText = typeof rawOutputText === 'string' && rawOutputText.trim() === ''
                            ? undefined
                            : rawOutputText;
                        const prefersFinalOutput = completedResult.state?._finalOutputSource === 'error_handler';
                        outputText = prefersFinalOutput
                            ? (finalOutputText ?? normalizedRawOutputText ?? '')
                            : (normalizedRawOutputText ?? finalOutputText ?? '');
                    }
                    if (details?.toolCall) {
                        (0, agentToolRunResults_1.saveAgentToolRunResult)(details.toolCall, completedResultWithAgentToolInvocation);
                    }
                    return outputText;
                }
                finally {
        ...
        (0, agentToolSourceRegistry_1.registerAgentToolSourceAgent)(agentTool, this);
        return agentTool;
    }
    /**
    ...
    async getMcpTools(runContext) {
        if (this.mcpServers.length > 0) {
            const includeServerInToolNames = this.mcpConfig.includeServerInToolNames === true;
    ...
    async getMcpToolReservedNames(runContext) {
        const reservedToolNames = new Set(this.tools
    ...
    processFinalOutput(output) {
        ...
        if (typeof this.outputType === 'object') {
            ...
            if ((0, typeGuards_1.isZodObject)(this.outputType)) {
