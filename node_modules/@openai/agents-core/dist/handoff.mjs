...
export class Handoff {
    ...
    clone(overrides = {}) {
        ...
        const onInvokeHandoff = overrides.onInvokeHandoff ??
            (overrides.agent
                ? async (context, args) => {
...
export function handoff(agent, config = {}) {
    ...
    if (hasInputType && config.onHandoff == null) {
        throw new UserError('You must provide `onHandoff` when `inputType` is provided.');
    }
    ...
    if (hasInputType) {
        const result = getSchemaAndParserFromInputType(inputType, handoff.toolName, { strict: true });
