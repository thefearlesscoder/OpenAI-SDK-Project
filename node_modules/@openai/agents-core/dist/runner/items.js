...
exports.stripReasoningItemIdForPolicy = stripReasoningItemIdForPolicy;
...
function stripReasoningItemIdForPolicy(item, reasoningItemIdPolicy) {
    if (!shouldOmitReasoningItemIds(reasoningItemIdPolicy) ||
        !item ||
        typeof item !== 'object' ||
        item.type !== 'reasoning' ||
        !('id' in item)) {
...
function extractOutputItemsFromRunItems(items, reasoningItemIdPolicy) {
    return items
        ...
        .map((item) => {
        const rawItem = withoutNullStatus(item.rawItem);
...
function withoutNullStatus(item) {
    if (!item ||
        typeof item !== 'object' ||
        !('status' in item) ||
        item.status !== null) {
...
function dropOrphanToolCalls(items, options) {
    ...
    return dropReasoningItemsPrecedingDroppedCalls(items, droppedIndexes, pruningIndexes);
}
function dropReasoningItemsPrecedingDroppedCalls(items, droppedIndexes, pruningIndexes) {
    const dropReasoning = new Set();
