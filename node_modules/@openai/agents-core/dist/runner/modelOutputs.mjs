 ...
 */
export function processModelResponse(modelResponse, agent, tools, handoffs, priorItems = [], toolNotFoundBehavior = 'raise_error') {
    const items = [];
    const runHandoffs = [];
    const runFunctions = [];
...
export async function processModelResponseAsync(modelResponse, agent, tools, handoffs, state, priorItems = [], toolNotFoundBehavior = 'raise_error') {
    const clientToolSearchTool = getClientToolSearchHelper(tools);
    ...
    if (!hasCustomClientToolSearchExecutor || !hasRelevantClientToolSearchCall) {
        return processModelResponse(modelResponse, agent, tools, handoffs, priorItems, toolNotFoundBehavior);
    }
    const items = [];
    const runHandoffs = [];
    const runFunctions = [];
