...
async function resumeInterruptedTurn(options) {
    const { state, runner, toolErrorFormatter, agentToolParentRunConfig, onStepItems, } = options;
    const turnResult = await (0, turnResolution_1.resolveInterruptedTurn)(state._currentAgent, state._originalInput, state._generatedItems, state._lastTurnResponse, state._lastProcessedResponse, runner, state, toolErrorFormatter, agentToolParentRunConfig);
    applyTurnResult({
        state,
        turnResult,
