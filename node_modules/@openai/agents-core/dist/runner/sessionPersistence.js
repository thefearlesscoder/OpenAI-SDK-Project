...
exports.createSessionPersistenceTracker = createSessionPersistenceTracker;
...
function createSessionPersistenceTracker(options) {
...
function collectPersistableFilteredItems(options) {
    ...
    for (let i = 0; i < filteredItems.length; i++) {
        ...
        if (source && typeof source === 'object') {
            ...
            const sourceKey = (0, items_1.getAgentInputItemKey)(source);
        ...
        const filteredKey = (0, items_1.getAgentInputItemKey)(filteredItem);
...
async function prepareInputItemsWithSession(input, session, sessionInputCallback, options) {
    ...
    const newInputItems = (0, items_1.toAgentInputList)(input);
    if (!sessionInputCallback) {
        const historyForModelInput = history.map((item) => prepareHistoryItemForModelInput(session, item, reasoningItemIdPolicy));
        const preparedInput = includeHistoryInPreparedInput
            ? (0, items_1.dropOrphanToolCalls)([...historyForModelInput, ...newInputItems], {
    ...
    const historyRefs = (0, items_1.buildAgentInputPool)(historySnapshot);
    const newInputRefs = (0, items_1.buildAgentInputPool)(newInputSnapshot);
    const historyIndexes = new Set();
    const appended = [];
    for (const [index, item] of combined.entries()) {
        const historyKey = getHistoryItemModelInputKey(session, item, reasoningItemIdPolicy);
        const newInputKey = (0, items_1.getAgentInputItemKey)(item);
        if ((0, items_1.removeAgentInputFromPool)(newInputRefs, item)) {
            decrementCount(newInputCounts, newInputKey);
            appended.push(item);
            continue;
        }
        if ((0, items_1.removeAgentInputFromPool)(historyRefs, item)) {
    ...
    if (preserveDroppedNewItems &&
        ...
        logger_1.default.warn('sessionInputCallback dropped all new inputs in a server-managed conversation; original turn inputs were restored to avoid losing the API delta. Keep at least one new item or omit conversationId if you intended to drop them.');
    }
    const prunedPreparedItems = includeHistoryInPreparedInput
        ? (0, items_1.dropOrphanToolCalls)(prepareHistoryItemsForModelInput(session, preparedItems, historyIndexes, reasoningItemIdPolicy), { pruningIndexes: historyIndexes })
        : preparedItems;
    return {
...
function prepareHistoryItemsForModelInput(session, items, historyIndexes, reasoningItemIdPolicy) {
    if (historyIndexes.size === 0) {
        return items;
    }
    return items.map((item, index) => historyIndexes.has(index)
        ? prepareHistoryItemForModelInput(session, item, reasoningItemIdPolicy)
        : item);
}
function prepareHistoryItemForModelInput(session, item, reasoningItemIdPolicy) {
    const prepared = session.prepareHistoryItemForModelInput?.(item) ?? item;
    return (0, items_1.stripReasoningItemIdForPolicy)(prepared, reasoningItemIdPolicy);
}
function getHistoryItemModelInputKey(session, item, reasoningItemIdPolicy) {
    return (0, items_1.getAgentInputItemKey)(prepareHistoryItemForModelInput(session, item, reasoningItemIdPolicy));
}
...
function sanitizeValueForSession(value, context = {}) {
    ...
    const binary = (0, binary_1.toUint8ArrayFromBinary)(value);
    if (binary) {
...
function shouldStripIdForProtocolItem(record) {
    switch (record.type) {
        case 'function_call':
...
function hasToolSearchCallId(record) {
...
async function persistRunItemsToSession(options) {
    ...
    const itemsToSave = [
        ...extraInputItems,
        ...(0, items_1.extractOutputItemsFromRunItems)(newRunItems, session.preserveReasoningItemIdsForPersistence?.() === true
            ? undefined
...
function buildItemFrequencyMap(items, options) {
    const counts = new Map();
    for (const item of items) {
        const key = options?.prepareForModelInput && options.session
            ? getHistoryItemModelInputKey(options.session, item, options.reasoningItemIdPolicy)
            : (0, items_1.getAgentInputItemKey)(item);
