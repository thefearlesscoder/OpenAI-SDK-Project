...
function getFunctionToolIdentity(toolRun) {
    return (0, toolIdentity_1.getFunctionToolQualifiedName)(toolRun.tool) ?? toolRun.tool.name;
}
function cloneForCustomDataContext(value) {
    try {
 ...
 */
async function executeFunctionToolCalls(agent, toolRuns, runner, state, toolErrorFormatter, agentToolParentRunConfig) {
    const deps = {
        agent,
        runner,
        state,
    ...
    const executeToolRun = async (toolRun) => {
    ...
    try {
        const results = await executeToolRunsWithConcurrency(toolRuns, getMaxFunctionToolConcurrency(agentToolParentRunConfig?.toolExecution ?? runner.config.toolExecution), executeToolRun);
        return results;
    }
    catch (e) {
...
function getMaxFunctionToolConcurrency(toolExecution) {
    return toolExecution?.maxFunctionToolConcurrency ?? undefined;
}
function shouldRunPreApprovalInputGuardrails(deps) {
    return ((deps.agentToolParentRunConfig?.toolExecution ??
        deps.runner.config.toolExecution)?.preApprovalInputGuardrails === true);
}
async function executeToolRunsWithConcurrency(toolRuns, maxConcurrency, executeToolRun) {
    ...
    await Promise.allSettled(Array.from({ length: workerCount }, async () => worker()));
...
function parseToolArguments(toolRun) {
    ...
    try {
        ...
        logger_1.default.debug(`Failed to parse tool arguments for ${toolName}: ${error}`);
...
function buildApprovalRequestResult(deps, toolRun) {
    return {
        ...
        tool: toolRun.tool,
        runItem: new items_1.RunToolApprovalItem(toolRun.toolCall, deps.agent, getFunctionToolIdentity(toolRun)),
    };
}
...
async function buildApprovalRejectionResult(deps, toolRun) {
    ...
    return withToolFunctionSpan(runner, traceToolName, async (span) => {
        ...
        const traceErrorMessage = runner.config.traceIncludeSensitiveData
            ? response
            : approvalRejection_1.TOOL_APPROVAL_REJECTION_MESSAGE;
        span?.setError({
...
function buildInputGuardrailRejectionResult(deps, toolRun, message) {
    return {
        ...
        tool: toolRun.tool,
        output: message,
        runItem: new items_1.RunToolCallOutputItem(getToolCallOutputItem(toolRun.toolCall, message), deps.agent, message),
    };
}
async function runFunctionToolInputGuardrails({ guardrails, context, agent, toolCall, onResult, }) {
    return (0, toolGuardrails_1.runToolInputGuardrails)({
        guardrails,
        context,
        agent,
        toolCall,
...
async function runApprovedFunctionTool(deps, toolRun, parsedInput) {
    ...
    return withToolFunctionSpan(runner, traceToolName, async (span) => {
        ...
        try {
            ...
            if (inputGuardrailResult.type === 'reject') {
                ...
                const resumeState = state.getPendingAgentToolRun(toolName, toolRun.toolCall.callId);
                const toolDetails = {
                    toolCall: toolRun.toolCall,
                    resumeState,
                    [tool_1.FUNCTION_TOOL_PARSED_INPUT_CALLBACK]: (input) => {
                        executedInput = cloneForCustomDataContext(input);
                    },
                };
                (0, agentToolRunConfig_1.setAgentToolParentRunConfigOnDetails)(toolDetails, agentToolParentRunConfig ?? runner.config);
            ...
            const customData = await (0, customData_1.maybeExtractToolOutputCustomData)(toolRun.tool.customDataExtractor, {
                runContext: state._context,
                tool: toolRun.tool,
            ...
            const functionResult = {
                ...
                tool: toolRun.tool,
                output: toolOutput,
                runItem: new items_1.RunToolCallOutputItem(rawItem, agent, toolOutput, customData),
            };
            ...
            if (nestedRunResult) {
                ...
                if (nestedInterruptions.length > 0) {
                    ...
                    state.setPendingAgentToolRun(toolName, toolRun.toolCall.callId, JSON.stringify(nestedRunStateJson));
                }
                else {
                    state.clearPendingAgentToolRun(toolName, toolRun.toolCall.callId);
                }
    ...
    if (onApproval) {
        ...
        if (decision.approve === true) {
            ...
            const reason = typeof decision.reason === 'string' && decision.reason.length > 0
                ? decision.reason
                : undefined;
            runContext.rejectTool(approvalItem, reason === undefined ? undefined : { message: reason });
        }
    }
...
async function executeApplyPatchOperations(agent, actions, runner, runContext, customLogger = undefined, toolErrorFormatter) {
    ...
    for (const action of actions) {
        ...
        const applyPatchItem = await withToolFunctionSpan(runner, applyPatchTool.name, async (span) => {
            ...
            const customData = await (0, customData_1.maybeExtractToolOutputCustomData)(applyPatchTool.customDataExtractor, {
                runContext,
                tool: applyPatchTool,
            ...
            return new items_1.RunToolCallOutputItem(rawItem, agent, output, customData);
        });
...
async function executeComputerActions(agent, actions, runner, runContext, customLogger = undefined, toolErrorFormatter) {
    ...
    for (const action of actions) {
        ...
        const getRejectionMessage = async () => {
            ...
            cachedRejectionMessage = await (0, approvalRejection_1.resolveApprovalRejectionMessage)({
                runContext,
                toolType: 'computer',
        ...
        const computerItem = await withToolFunctionSpan(runner, COMPUTER_TRACE_NAME, async (span) => {
            if (span && runner.config.traceIncludeSensitiveData) {
            ...
            const customData = await (0, customData_1.maybeExtractToolOutputCustomData)(computerTool.customDataExtractor, {
                runContext,
                tool: computerTool,
            ...
            return new items_1.RunToolCallOutputItem(rawItem, agent, imageUrl, customData);
        });
...
async function executeHandoffCalls(agent, originalInput, preStepItems, newStepItems, newResponse, runHandoffs, runner, runContext) {
    ...
    if (runHandoffs.length > 1) {
        const ignoredCallIds = new Set(runHandoffs.slice(1).map((handoff) => handoff.toolCall.callId));
        // Drop ignored handoff requests from the step so they never persist to history.
        newStepItems = newStepItems.filter((item) => !(item instanceof items_1.RunHandoffCallItem &&
...
async function checkForFinalOutputFromTools(agent, toolResults, state, additionalInterruptions = []) {
    ...
    if (typeof toolUseBehavior === 'object') {
        const stoppingTool = toolResults.find((r) => {
            return toolUseBehavior.stopAtToolNames.some((toolName) => (0, toolIdentity_1.matchesFunctionToolName)(r.tool, toolName));
        });
