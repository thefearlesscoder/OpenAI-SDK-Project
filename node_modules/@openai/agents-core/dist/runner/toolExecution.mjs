import { setAgentToolParentRunConfigOnDetails } from "../agentToolRunConfig.mjs";
import { consumeAgentToolRunResult } from "../agentToolRunResults.mjs";
import { ToolCallError, ToolTimeoutError, UserError } from "../errors.mjs";
import { getTransferMessage } from "../handoff.mjs";
import { RunHandoffCallItem, RunHandoffOutputItem, RunMessageOutputItem, RunToolApprovalItem, RunToolCallOutputItem, } from "../items.mjs";
 ...
 */
export async function executeFunctionToolCalls(agent, toolRuns, runner, state, toolErrorFormatter, agentToolParentRunConfig) {
    const deps = {
        agent,
        runner,
        state,
...
function buildApprovalRequestResult(deps, toolRun) {
    return {
        ...
        tool: toolRun.tool,
        runItem: new RunToolApprovalItem(toolRun.toolCall, deps.agent, getFunctionToolIdentity(toolRun)),
    };
}
...
function buildInputGuardrailRejectionResult(deps, toolRun, message) {
    return {
        ...
        tool: toolRun.tool,
        output: message,
        runItem: new RunToolCallOutputItem(getToolCallOutputItem(toolRun.toolCall, message), deps.agent, message),
    };
}
        ...
        try {
            ...
            if (inputGuardrailResult.type === 'reject') {
                ...
                const toolDetails = {
                    toolCall: toolRun.toolCall,
                    resumeState,
                    [FUNCTION_TOOL_PARSED_INPUT_CALLBACK]: (input) => {
                        executedInput = cloneForCustomDataContext(input);
                    },
                };
                setAgentToolParentRunConfigOnDetails(toolDetails, agentToolParentRunConfig ?? runner.config);
            ...
            const customData = await maybeExtractToolOutputCustomData(toolRun.tool.customDataExtractor, {
                runContext: state._context,
                tool: toolRun.tool,
            ...
            const functionResult = {
                ...
                tool: toolRun.tool,
                output: toolOutput,
                runItem: new RunToolCallOutputItem(rawItem, agent, toolOutput, customData),
            };
...
export async function executeApplyPatchOperations(agent, actions, runner, runContext, customLogger = undefined, toolErrorFormatter) {
    ...
    for (const action of actions) {
        ...
        const applyPatchItem = await withToolFunctionSpan(runner, applyPatchTool.name, async (span) => {
            ...
            const customData = await maybeExtractToolOutputCustomData(applyPatchTool.customDataExtractor, {
                runContext,
                tool: applyPatchTool,
...
export async function executeComputerActions(agent, actions, runner, runContext, customLogger = undefined, toolErrorFormatter) {
    ...
    for (const action of actions) {
        ...
        const computerItem = await withToolFunctionSpan(runner, COMPUTER_TRACE_NAME, async (span) => {
            ...
            const customData = await maybeExtractToolOutputCustomData(computerTool.customDataExtractor, {
                runContext,
                tool: computerTool,
...
export async function executeHandoffCalls(agent, originalInput, preStepItems, newStepItems, newResponse, runHandoffs, runner, runContext) {
    ...
    if (runHandoffs.length > 1) {
        ...
        newStepItems = newStepItems.filter((item) => !(item instanceof RunHandoffCallItem &&
...
export async function checkForFinalOutputFromTools(agent, toolResults, state, additionalInterruptions = []) {
    ...
    if (typeof toolUseBehavior === 'object') {
        const stoppingTool = toolResults.find((r) => {
            return toolUseBehavior.stopAtToolNames.some((toolName) => matchesFunctionToolName(r.tool, toolName));
        });
