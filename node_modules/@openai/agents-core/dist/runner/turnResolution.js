...
async function resolveToolNotFoundMessage(state, toolRun, toolErrorFormatter) {
    ...
    try {
        ...
        logger_1.default.warn(`toolErrorFormatter threw while formatting tool not found: ${message}`);
    }
    return defaultMessage;
}
 ...
 */
async function resolveInterruptedTurn(agent, originalInput, originalPreStepItems, newResponse, processedResponse, runner, state, toolErrorFormatter, agentToolParentRunConfig) {
    ...
    const functionToolRuns = processedResponse.functions.filter((run) => {
        ...
        const isPendingNested = state.hasPendingAgentToolRun((0, toolIdentity_1.getFunctionToolQualifiedName)(run.tool) ?? run.tool.name, callId);
    ...
    const functionResults = await (0, toolExecution_1.executeFunctionToolCalls)(agent, functionToolRuns, runner, state, toolErrorFormatter, agentToolParentRunConfig);
 ...
 */
async function resolveTurnAfterModelResponse(agent, originalInput, originalPreStepItems, newResponse, processedResponse, runner, state, toolErrorFormatter, agentToolParentRunConfig, errorHandlers) {
    ...
    const [functionResults, computerResults] = await Promise.all([
        (0, toolExecution_1.executeFunctionToolCalls)(agent, processedResponse.functions, runner, state, toolErrorFormatter, agentToolParentRunConfig),
    ...
    if (!hasPendingToolsOrApprovals && messageItems.length > 0) {
        ...
        if (refusal && typeof potentialFinalOutput === 'undefined') {
            ...
            return new steps_1.SingleStepResult(originalInput, newResponse, preStepItems, newItems, { type: 'next_step_final_output', output: outputText });
        }
    }
