import { z } from 'zod';
import { ModelBehaviorError, ModelRefusalError } from "../errors.mjs";
import { RunMessageOutputItem, RunToolApprovalItem, RunToolCallOutputItem, } from "../items.mjs";
import logger from "../logger.mjs";
import { getRefusalFromOutputMessage, getTextFromOutputMessage, } from "../utils/messages.mjs";
...
import { checkForFinalOutputFromTools, executeApplyPatchOperations, executeComputerActions, executeFunctionToolCalls, executeHandoffCalls, executeShellActions, collectInterruptions, getToolCallOutputItem, } from "./toolExecution.mjs";
import { handleHostedMcpApprovals } from "./mcpApprovals.mjs";
import { getFunctionToolQualifiedName } from "../toolIdentity.mjs";
import { createRunErrorFinalOutputItem, formatRunErrorFinalOutput, resolveRunErrorHandler, } from "./errorHandlers.mjs";
...
async function resolveToolNotFoundMessage(state, toolRun, toolErrorFormatter) {
    ...
    try {
        ...
        if (typeof formattedMessage !== 'undefined') {
            logger.warn('toolErrorFormatter returned a non-string value. Falling back to the default tool not found message.');
        }
    }
    catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        logger.warn(`toolErrorFormatter threw while formatting tool not found: ${message}`);
    }
    return defaultMessage;
}
async function buildToolNotFoundOutputItems(agent, state, toolRuns, toolErrorFormatter) {
    ...
    for (const toolRun of toolRuns) {
        ...
        items.push(new RunToolCallOutputItem(getToolCallOutputItem(toolRun.toolCall, message), agent, message));
    }
    return items;
}
...
async function executeShellAndApplyPatchActionsInOrder(args) {
    ...
    for (const action of orderShellAndApplyPatchActions(args.sourceItems, args.shellActions, args.applyPatchActions)) {
        const items = action.type === 'shell'
            ? await executeShellActions(args.agent, [action.action], args.runner, args.state._context, undefined, args.toolErrorFormatter)
            : await executeApplyPatchOperations(args.agent, [action.action], args.runner, args.state._context, undefined, args.toolErrorFormatter);
        results.push(...items);
    }
    return results;
}
 ...
 */
export async function resolveInterruptedTurn(agent, originalInput, originalPreStepItems, newResponse, processedResponse, runner, state, toolErrorFormatter, agentToolParentRunConfig) {
    ...
    const functionToolRuns = processedResponse.functions.filter((run) => {
        ...
        const isPendingNested = state.hasPendingAgentToolRun(getFunctionToolQualifiedName(run.tool) ?? run.tool.name, callId);
    ...
    const functionResults = await executeFunctionToolCalls(agent, functionToolRuns, runner, state, toolErrorFormatter, agentToolParentRunConfig);
    ...
    const computerResults = pendingComputerActions.length > 0
        ? await executeComputerActions(agent, pendingComputerActions, runner, state._context, undefined, toolErrorFormatter)
        : [];
    const shellAndApplyPatchResults = shellRuns.length > 0 || applyPatchRuns.length > 0
        ...
        : [];
    const pendingFunctionToolsNotFound = filterPendingActions(processedResponse.functionToolsNotFound ?? [], {
    ...
    const additionalInterruptions = collectInterruptions([], [...computerResults, ...shellAndApplyPatchResults]);
 ...
 */
export async function resolveTurnAfterModelResponse(agent, originalInput, originalPreStepItems, newResponse, processedResponse, runner, state, toolErrorFormatter, agentToolParentRunConfig, errorHandlers) {
    ...
    const [functionResults, computerResults] = await Promise.all([
        executeFunctionToolCalls(agent, processedResponse.functions, runner, state, toolErrorFormatter, agentToolParentRunConfig),
        executeComputerActions(agent, processedResponse.computerActions, runner, state._context, undefined, toolErrorFormatter),
    ]);
    const shellAndApplyPatchResults = processedResponse.shellActions.length > 0 ||
        ...
        : [];
    const toolNotFoundResults = await buildToolNotFoundOutputItems(agent, state, processedResponse.functionToolsNotFound ?? [], toolErrorFormatter);
    for (const result of functionResults) {
        if (result.type === 'function_output' &&
            Array.isArray(result.interruptions) &&
            result.interruptions.length > 0) {
    ...
    if (!hasPendingToolsOrApprovals && messageItems.length > 0) {
        const refusal = getRefusalFromOutputMessage(messageItems[messageItems.length - 1].rawItem);
        if (refusal && typeof potentialFinalOutput === 'undefined') {
            ...
            const outputText = formatRunErrorFinalOutput(agent, handlerResult.finalOutput);
            if (handlerResult.includeInHistory !== false) {
                newItems.push(createRunErrorFinalOutputItem(agent, outputText));
            }
            return new SingleStepResult(originalInput, newResponse, preStepItems, newItems, { type: 'next_step_final_output', output: outputText });
        }
    }
