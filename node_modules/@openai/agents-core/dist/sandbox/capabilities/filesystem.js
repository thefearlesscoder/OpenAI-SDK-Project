...
const transport_1 = require("./transport.js");
...
function renderImageForTextTransport(output) {
    ...
    if (!(0, typeGuards_1.isRecord)(image)) {
    ...
    if (imageRecord.data instanceof Uint8Array) {
        const mediaType = typeof imageRecord.mediaType === 'string'
            ? imageRecord.mediaType
            : 'application/octet-stream';
        return `data:${mediaType};base64,${(0, base64_1.encodeUint8ArrayToBase64)(imageRecord.data)}`;
    }
    return 'No image data was returned by the sandbox session.';
}
...
function parseStructuredOperations(payloads) {
    ...
    for (const payload of payloads) {
        const parsed = protocol_1.ApplyPatchOperation.safeParse(payload);
        if (!parsed.success) {
            return {
                ok: false,
...
class FilesystemCapability extends base_1.Capability {
    type = 'filesystem';
    configureTools;
    constructor(args = {}) {
        super();
    ...
    tools() {
        ...
        if (!editor) {
            throw new errors_1.UserError('Filesystem sandbox sessions must provide createEditor().');
        }
        const tools = [];
        const viewImage = async (path) => {
            if (!session.viewImage) {
                throw new errors_1.UserError('Filesystem sandbox sessions must provide viewImage().');
            }
            try {
                return await (0, spans_1.withSandboxSpan)('sandbox.view_image', {
                    path,
                    run_as: this._runAs,
        ...
        if ((0, transport_1.supportsStructuredToolOutputTransport)(this._modelInstance)) {
            tools.push((0, tool_1.tool)({
                name: 'view_image',
                description: 'Returns an image output from a local path in the sandbox workspace.',
                parameters: zod_1.z.object({
                    path: zod_1.z.string().describe('Local filesystem path to an image file'),
                }),
        ...
        if ((0, transport_1.supportsApplyPatchTransport)(this._modelInstance)) {
            tools.push((0, tool_1.applyPatchTool)({
                name: 'apply_patch',
                editor,
            }));
        }
