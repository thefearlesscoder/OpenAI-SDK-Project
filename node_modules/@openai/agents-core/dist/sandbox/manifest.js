...
exports.Manifest = exports.Environment = void 0;
exports.cloneManifest = cloneManifest;
exports.normalizeRelativePath = normalizeRelativePath;
exports.renderManifestDescription = renderManifestDescription;
const entries_1 = require("./entries/index.js");
const pathGrants_1 = require("./pathGrants.js");
...
class Manifest {
    ...
    constructor(init = {}) {
        ...
        this.users = (init.users ?? []).map((user) => (0, users_1.normalizeUser)(user));
        this.groups = (init.groups ?? []).map((group) => (0, users_1.normalizeGroup)(group));
        this.extraPathGrants = (init.extraPathGrants ?? []).map((grant) => (0, pathGrants_1.normalizePathGrant)(grant));
        this.remoteMountCommandAllowlist = [
            ...(init.remoteMountCommandAllowlist ??
                remoteMountCommandAllowlist_1.DEFAULT_REMOTE_MOUNT_COMMAND_ALLOWLIST),
        ];
    }
    validatedEntries() {
    ...
    *iterEntries() {
        ...
        while (queue.length > 0) {
            ...
            yield current;
            if (!(0, entries_1.isDir)(current.entry) || !current.entry.children) {
                continue;
            }
...
function normalizeRelativePath(path) {
    ...
    if ((0, posixPath_1.hasBackslashPathSeparator)(path)) {
        throw new Error(`Manifest path "${path}" must use "/" separators.`);
    }
    if (trimmed.startsWith('/')) {
        throw new Error(`Manifest path "${path}" must be relative.`);
    }
    if ((0, posixPath_1.hasParentPathSegment)(path)) {
        throw new Error(`Manifest path "${path}" must not contain parent segments.`);
    }
    const normalized = (0, posixPath_1.normalizePosixPath)(path);
    return normalized === '.' ? '' : normalized;
}
function normalizeGitRepoSubpath(repo, subpath) {
    const trimmed = subpath.trim();
    let reason;
    if (subpath === '' || (0, posixPath_1.normalizePosixPath)(trimmed) === '.') {
        return '';
    }
    if (!trimmed) {
        reason = 'empty';
    }
    else if ((0, posixPath_1.hasBackslashPathSeparator)(subpath) ||
        /^[A-Za-z]:($|\/)/.test(trimmed)) {
        reason = 'windows_path';
    }
    ...
    else if ((0, posixPath_1.hasParentPathSegment)(subpath)) {
        reason = 'parent_traversal';
    }
    if (reason) {
        throw new errors_1.SandboxGitSubpathError(`git_repo subpath must be a relative path inside the repository: ${subpath}`, { repo, subpath, reason });
    }
    return (0, posixPath_1.normalizePosixPath)(trimmed);
}
function renderManifestDescription(manifest, options = {}) {
...
function relativePathWithinRoot(root, path) {
    ...
    return (0, posixPath_1.relativePosixPathWithinRoot)(normalizedRoot, normalizedPath);
}
function collectMountTargets(manifest) {
    const targets = [];
    for (const iterEntry of manifest.iterEntries()) {
        if (!(0, entries_1.isMount)(iterEntry.entry)) {
            continue;
        }
        targets.push({
            ...iterEntry,
...
function normalizeEntry(entry, logicalPath, seenPaths) {
    ...
    if (normalized.permissions !== undefined) {
        normalized.permissions = (0, permissions_1.normalizePermissions)(normalized.permissions);
    }
    if (normalized.group !== undefined) {
        normalized.group = (0, users_1.normalizeEntryGroup)(normalized.group);
    }
    if ((0, entries_1.isDir)(normalized) && normalized.children) {
        normalized.children = normalizeEntries(normalized.children, logicalPath, seenPaths);
    }
    if ((0, entries_1.isGitRepo)(normalized)) {
    ...
    if ((0, entries_1.isMount)(normalized)) {
...
function normalizeMountStrategy(strategy) {
    if (!(0, typeGuards_1.isRecord)(strategy)) {
    ...
    if ((0, typeGuards_1.isRecord)(normalized.pattern)) {
        ...
        if ((0, typeGuards_1.isRecord)(normalized.pattern.options)) {
...
function normalizeTypedMountProvider(entry) {
    if (!(0, entries_1.isMount)(entry) || entry.type === 'mount') {
        return;
    }
    switch (entry.type) {
        case 's3_mount':
            setTypedMountProviderConfig(entry, 's3', typedMountConfig({ bucket: entry.bucket }, {
                prefix: entry.prefix,
                region: entry.region,
            ...
            return;
        case 'gcs_mount':
            setTypedMountProviderConfig(entry, 'gcs', typedMountConfig({ bucket: entry.bucket }, {
                prefix: entry.prefix,
                region: entry.region,
                endpointUrl: entry.endpointUrl,
            }));
            return;
        case 'r2_mount':
            setTypedMountProviderConfig(entry, 'r2', typedMountConfig({ bucket: entry.bucket }, {
                prefix: entry.prefix,
        ...
        case 'azure_blob_mount':
            setTypedMountProviderConfig(entry, 'azure_blob', typedMountConfig({ container: entry.container }, {
                prefix: entry.prefix,
            ...
            return;
        case 'box_mount':
            setTypedMountProviderConfig(entry, 'box', typedMountConfig({}, {
                path: entry.path,
            ...
            return;
        case 's3_files_mount':
            setTypedMountProviderConfig(entry, 's3_files', typedMountConfig({ fileSystemId: entry.fileSystemId }, {
...
function setTypedMountProviderConfig(entry, provider, config) {
...
function typedMountConfig(required, optional = {}) {
