...
exports.resolveMemoryRolloutId = resolveMemoryRolloutId;
exports.buildMemoryRolloutPayload = buildMemoryRolloutPayload;
exports.validateRolloutId = validateRolloutId;
exports.normalizeRolloutSlug = normalizeRolloutSlug;
exports.renderPhaseOnePrompt = renderPhaseOnePrompt;
...
function resolveMemoryRolloutId(identity) {
    return validateRolloutId(identity.conversationId ??
        identity.sdkSessionId ??
        identity.groupId ??
        identity.fallbackId);
}
function buildMemoryRolloutPayload(state, args) {
    const finalOutput = finalOutputForMemory(state);
    const interruptions = interruptionsForMemory(state).map((item) => sanitizeForMemoryJson(item.rawItem));
    const payload = {
        updated_at: utcIsoTimestamp(),
        rollout_id: args.rolloutId,
        input: sanitizeForMemoryJson(sanitizeMemoryItems((0, items_1.toAgentInputList)(args.inputOverride ?? state._originalInput))),
        generated_items: sanitizeForMemoryJson(sanitizeMemoryItems((0, items_1.extractOutputItemsFromRunItems)(state._generatedItems, state._reasoningItemIdPolicy))),
        terminal_metadata: terminalMetadataForMemory(state, args.exception, finalOutput),
    };
...
function validateRolloutId(value) {
    const normalized = value.trim();
    if (!/^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$/.test(normalized)) {
        throw new Error("Sandbox memory rollout ID must be a file-safe ID containing only letters, numbers, '.', '_', or '-'.");
    }
    return normalized;
}
function normalizeRolloutSlug(value) {
...
function renderPhaseOnePrompt(rolloutContents) {
    ...
    const terminalMetadata = payloads.length === 1
...
function shouldIncludeMemoryItem(item) {
...
function finalOutputForMemory(state) {
    if (state._currentStep?.type !== 'next_step_final_output') {
        return null;
    }
    return state._currentStep.output;
}
function interruptionsForMemory(state) {
    return state.getInterruptions();
}
function terminalMetadataForMemory(state, exception, finalOutput) {
    if (finalOutput !== null) {
        return {
...
function terminalMetadataForException(exception) {
    const exceptionType = exceptionName(exception);
    let terminalState = 'failed';
    if (exception instanceof errors_1.MaxTurnsExceededError) {
        terminalState = 'max_turns_exceeded';
    }
    else if (exception instanceof errors_1.InputGuardrailTripwireTriggered ||
        exception instanceof errors_1.OutputGuardrailTripwireTriggered ||
        exception instanceof errors_1.ToolInputGuardrailTripwireTriggered ||
        exception instanceof errors_1.ToolOutputGuardrailTripwireTriggered ||
        exceptionType.includes('Guardrail')) {
        terminalState = 'guardrail_tripped';
    }
    else if (exceptionType === 'AbortError' ||
...
function exceptionName(exception) {
...
function stringifySortedJson(value) {
    return (0, stableJson_1.stableJsonPrettyStringify)(value, {
...
function sanitizeForMemoryJson(value) {
    try {
