...
import { resolveSandboxArchiveLimits, } from "../../client.mjs";
import { SandboxArchiveError } from "../../errors.mjs";
import { isNoopSnapshotSpec, } from "../../snapshot.mjs";
...
import { isHostPathStrictlyWithinRoot, isHostPathWithinRoot, relativeHostPathEscapesRoot, } from "../../shared/hostPath.mjs";
import { defaultLocalSnapshotBaseDir } from "./localSnapshotPaths.mjs";
import { serializeManifest } from "./manifestPersistence.mjs";
...
const SNAPSHOT_FILE_WRITE_FLAGS = constants.O_WRONLY |
    constants.O_CREAT |
    constants.O_TRUNC |
    constants.O_NOFOLLOW;
export async function createLocalSnapshot(state, spec) {
...
export async function createRemoteSnapshot(state, spec) {
...
export async function persistLocalSnapshot(providerName, state, spec) {
    ...
    if (spec && spec.type !== 'local') {
        throw new UserError(`${providerName} does not support snapshot specs of type "${spec.type}".`);
    }
    const fingerprint = await computeLocalSnapshotFingerprint(state);
...
export function serializeLocalSnapshotSpec(spec) {
    if (!spec) {
        return null;
    }
    if (spec.type === 'remote') {
...
export function rehydrateLocalSnapshotSpec(serialized, configured) {
...
export async function replaceLocalSnapshot(state, snapshot) {
...
export async function canReuseLocalSnapshotWorkspace(state) {
...
export async function localSnapshotIsRestorable(state) {
...
export async function restoreLocalSnapshotToWorkspace(state, workspaceRootPath, options = {}) {
    if (!(await localSnapshotIsRestorable(state))) {
...
export async function computeLocalSnapshotFingerprint(state) {
    const hash = createHash('sha256');
    appendHashFrame(hash, 'version', LOCAL_SNAPSHOT_FINGERPRINT_VERSION);
    appendHashFrame(hash, 'manifest', stableJsonStringify(serializeManifest(state.manifest), {
        encodeBytes: (value) => ({
            type: 'Uint8Array',
...
export async function copyDirectory(sourceRoot, destinationRoot, excludedLogicalPaths = new Set(), currentRelativePath = '', expectedSourceStats, destinationRootPath = destinationRoot) {
    ...
    for (const entry of entries) {
        ...
        await assertSafeSnapshotDestinationPath(destinationRootPath, dirname(destinationPath));
...
export async function createWorkspaceArchive(sourceRoot, excludedLogicalPaths) {
...
export async function restoreWorkspaceArchive(data, destinationRoot, options = {}) {
...
function checkWorkspaceArchiveInputBytes(actual, limits) {
    ...
    if (limit != null && actual > limit) {
        throw new SandboxArchiveError('Workspace archive input size exceeds limit.', {
...
function checkWorkspaceArchiveMemberCount(actual, member, limits) {
    ...
    if (limit != null && actual > limit) {
        throw new SandboxArchiveError('Workspace archive member count exceeds limit.', {
...
function checkWorkspaceArchiveExtractedBytes(actual, member, limits) {
    ...
    if (limit != null && actual > limit) {
        throw new SandboxArchiveError('Workspace archive extracted size exceeds limit.', {
...
async function writeSafeArchiveFile(destinationRoot, relativePath, base64Data) {
    const destinationPath = safeArchivePath(destinationRoot, relativePath);
    await assertSafeSnapshotDestinationPath(destinationRoot, dirname(destinationPath));
    await mkdir(dirname(destinationPath), { recursive: true });
    await assertSafeSnapshotDestinationPath(destinationRoot, dirname(destinationPath));
...
export function isRemoteSnapshotSpec(spec) {
...
//# sourceMappingURL=localSnapshots.mjs.map