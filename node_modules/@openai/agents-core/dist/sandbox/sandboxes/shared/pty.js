...
exports.spawnInPseudoTerminal = spawnInPseudoTerminal;
...
`;
const checkedPtyBridgePythonExecutables = new Set();
function spawnInPseudoTerminal(executable, args, options = {}) {
    ...
    return (0, node_child_process_1.spawn)(pythonExecutable, ['-c', PTY_BRIDGE_SCRIPT, executable, ...args], {
        cwd: options.cwd,
        env,
        stdio: 'pipe',
    });
}
function assertPtyBridgePythonAvailable(pythonExecutable, options) {
    const cacheKey = `${pythonExecutable}\0${options.cwd ?? ''}\0${options.env.PATH ?? ''}`;
    if (checkedPtyBridgePythonExecutables.has(cacheKey)) {
        return;
    }
    const result = (0, node_child_process_1.spawnSync)(pythonExecutable, ['-c', PTY_BRIDGE_PYTHON_CHECK_SCRIPT], {
        cwd: options.cwd,
        env: options.env,
        stdio: 'pipe',
    });
    if (result.error || result.status !== 0) {
        throw new errors_1.SandboxConfigurationError('PTY support requires Python 3. Install python3 or set OPENAI_AGENTS_PYTHON to a Python 3 executable.', {
            pythonExecutable,
            cwd: options.cwd,
