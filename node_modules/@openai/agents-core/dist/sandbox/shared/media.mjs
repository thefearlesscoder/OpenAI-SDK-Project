...
export function assertViewImageByteLength(path, byteLength) {
    if (byteLength > MAX_VIEW_IMAGE_BYTES) {
        throw new UserError(`Image file exceeds the 10 MB limit: ${path}`);
    }
}
export function imageOutputFromBytes(path, bytes) {
...
export function sniffImageMediaType(bytes, path) {
    if (bytes[0] === 0x89 &&
        bytes[1] === 0x50 &&
        bytes[2] === 0x4e &&
        bytes[3] === 0x47) {
