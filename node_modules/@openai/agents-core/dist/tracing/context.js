...
exports.setTracingContextStorage = setTracingContextStorage;
 ...
 */
function setTracingContextStorage(storage) {
    try {
        const globalScope = globalThis;
        if (storage) {
            Object.defineProperty(globalScope, ALS_SYMBOL, {
...
function deferRestoreForStreamedRunResult(result, restore) {
...
function restoreAfterResolvedValue(result, restore) {
    if (!deferRestoreForStreamedRunResult(result, restore)) {
        restore();
    }
    return result;
}
function runWithScopedTraceContext(store, fn) {
    ...
    return asyncLocalStorage.run(store, () => {
        ...
        try {
            ...
            if (isPromiseLike(result)) {
                return result.then((resolved) => restoreAfterResolvedValue(resolved, restore), (error) => {
                    restore();
                    throw error;
                });
            }
