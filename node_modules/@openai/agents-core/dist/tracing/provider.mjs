...
import { defaultTracingIdGenerator, NOOP_TRACE_OR_SPAN_ID, } from "./utils.mjs";
...
export class TraceProvider {
    ...
    createSpan(spanOptions, parent) {
        ...
        if (!parent) {
            ...
            if (!currentTrace) {
                logger.debug('No active trace. Make sure to start a trace with `withTrace()` first. Returning NoopSpan.');
                return new NoopSpan(spanOptions.data, this.#multiProcessor);
            }
        ...
        logger.debug(`Creating span ${JSON.stringify(spanOptions.data)} with id ${spanId}`);
        return new Span({
            ...spanOptions,
            spanId,
            traceId,
