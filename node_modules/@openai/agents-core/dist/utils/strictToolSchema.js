...
exports.toOpenAIStrictToolSchema = toOpenAIStrictToolSchema;
exports.stripStrictNullsForJsonSchema = stripStrictNullsForJsonSchema;
exports.stripStrictNullsForZodSchema = stripStrictNullsForZodSchema;
const zodCompat_1 = require("./zodCompat.js");
function toOpenAIStrictToolSchema(schema) {
    return ensureStrictSchemaEntry(structuredClone(schema));
}
function ensureStrictSchemaEntry(entry) {
    ...
    if (record.type === 'object' &&
        ...
        const originalRequired = new Set(Array.isArray(record.required) ? record.required.map(String) : []);
...
function wrapNullableSchema(schema) {
...
function isSchemaNullable(schema) {
    ...
    for (const key of ['anyOf', 'oneOf', 'allOf']) {
        ...
        if (Array.isArray(entries) &&
            entries.some((entry) => typeof entry === 'object' &&
                entry !== null &&
                isSchemaNullable(entry))) {
            return true;
        }
    }
    return false;
}
function stripStrictNullsForJsonSchema(schema, value, optionalProperty = false) {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
    ...
    if (Array.isArray(value)) {
        ...
        if (Array.isArray(items)) {
            return value.map((entry, index) => stripStrictNullsForJsonSchema(items[index], entry));
        }
    ...
    for (const [key, propertySchema] of Object.entries(properties)) {
        const nextValue = stripStrictNullsForJsonSchema(propertySchema, normalized[key], !required.has(key));
...
function stripStrictNullsForZodSchema(schema, value) {
    ...
    const type = (0, zodCompat_1.readZodType)(inner);
    const def = (0, zodCompat_1.readZodDefinition)(inner);
    ...
    if (type === 'array' && Array.isArray(value)) {
        ...
        return value.map((entry) => stripStrictNullsForZodSchema(itemSchema, entry));
    }
    if (type === 'tuple' && Array.isArray(value)) {
        const items = coerceArray(def?.items);
        return value.map((entry, index) => stripStrictNullsForZodSchema(items[index], entry));
    }
    ...
    if (type === 'set' && Array.isArray(value)) {
        ...
        return value.map((entry) => stripStrictNullsForZodSchema(valueSchema, entry));
    }
    return value;
}
function unwrapZodOptionalNullable(schema) {
    ...
    while (current && typeof current === 'object' && !visited.has(current)) {
        visited.add(current);
        const type = (0, zodCompat_1.readZodType)(current);
...
function unwrapDecorators(schema) {
    let current = schema;
    const visited = new Set();
    while (current && typeof current === 'object' && !visited.has(current)) {
        visited.add(current);
...
function readZodUnionOptions(def) {
...
function zodSchemaAccepts(schema, value) {
    const candidate = unwrapDecorators(schema);
...
//# sourceMappingURL=strictToolSchema.js.map