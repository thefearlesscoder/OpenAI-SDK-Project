...
const strictToolSchema_1 = require("./strictToolSchema.js");
 ...
 */
function getSchemaAndParserFromInputType(inputType, name, options = {}) {
    ...
    if ((0, typeGuards_1.isZodObject)(inputType)) {
            ...
            if (fallbackSchema) {
                return {
                    schema: options.strict
                        ? (0, strictToolSchema_1.toOpenAIStrictToolSchema)(fallbackSchema)
                        : fallbackSchema,
                    parser: (rawInput) => inputType.parse(options.strict
                        ? (0, strictToolSchema_1.stripStrictNullsForZodSchema)(inputType, JSON.parse(rawInput))
                        : JSON.parse(rawInput)),
                };
            }
        ...
        if ((0, zodJsonSchemaCompat_1.hasJsonSchemaObjectShape)(formattedFunction.parameters)) {
            const fallbackSchema = buildJsonSchemaFromZod(inputType);
            if (fallbackSchema) {
                (0, zodJsonSchemaCompat_1.mergeJsonSchemaDescriptions)(formattedFunction.parameters, fallbackSchema);
            }
            return {
                schema: options.strict
                    ? (0, strictToolSchema_1.toOpenAIStrictToolSchema)(formattedFunction.parameters)
                    : formattedFunction.parameters,
                parser: options.strict
                    ? (rawInput) => inputType.parse((0, strictToolSchema_1.stripStrictNullsForZodSchema)(inputType, JSON.parse(rawInput)))
        ...
        return {
            schema: options.strict ? (0, strictToolSchema_1.toOpenAIStrictToolSchema)(inputType) : inputType,
            parser: options.strict
                ? (rawInput) => (0, strictToolSchema_1.stripStrictNullsForJsonSchema)(inputType, JSON.parse(rawInput))
                : parser,
        };
    }
