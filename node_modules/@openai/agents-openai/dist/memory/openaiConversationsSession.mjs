...
import { getDefaultOpenAIClient, getDefaultOpenAIKey } from "../defaults.mjs";
import { convertToOutputItem, getInputItems } from "../openaiResponsesModel.mjs";
import { OPENAI_SESSION_API, } from "./openaiSessionApi.mjs";
export async function startOpenAIConversationsSession(client) {
...
export class OpenAIConversationsSession {
    ...
    async addItems(items) {
        ...
        const sanitizedItems = stripConversationPersistenceMetadata(getInputItems(normalizedItems));
...
//  Internals
// --------------------------------------------------------------
function stripProviderModelForConversationPersistence(items) {
...
function stripConversationPersistenceMetadata(items) {
    return items.flatMap((item) => {
        ...
        const { providerData: _providerData, provider_data: _provider_data, ...rest } = record;
...
function isUnpersistableReasoningItem(item) {
    return (item.type === 'reasoning' &&
        typeof item.id !== 'string' &&
        typeof item.encrypted_content !== 'string');
}
function stripAssistantReplayMetadata(item) {
    if (Array.isArray(item) || item === null || typeof item !== 'object') {
        return item;
    }
    const record = item;
    ...
    const { id: _id, providerData: _providerData, provider_data: _provider_data, ...rest } = record;
    return rest;
}
...
//# sourceMappingURL=openaiConversationsSession.mjs.map