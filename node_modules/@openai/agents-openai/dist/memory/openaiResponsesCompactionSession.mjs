...
export class OpenAIResponsesCompactionSession {
    ...
    async getAllUnderlyingSessionItems() {
        return this.underlyingSession.getItems();
    }
    async replaceUnderlyingSessionItems({ outputItems, previousItems, }) {
        try {
            await this.underlyingSession.clearSession();
        }
        catch (error) {
            await this.restoreUnderlyingSessionItemsAfterFailedClear(previousItems, error);
            throw error;
        }
        try {
    ...
    async restoreUnderlyingSessionItemsAfterFailedClear(previousItems, error) {
        let currentItems;
        try {
            currentItems = await this.getAllUnderlyingSessionItems();
        }
        catch (inspectionError) {
            logger.warn('Failed to inspect session history after compaction replacement clear failed.', inspectionError);
            return;
        }
    ...
    async restoreUnderlyingSessionItems(previousItems, error, options = {}) {
        try {
            ...
            logger.warn('Failed to restore session history after compaction replacement failed.', restoreError);
            return;
        }
        logger.warn('Restored previous session history after compaction replacement failed.', error);
    }
...
function normalizeCompactionOutputItems(items) {
...
function isCompactionInputMessage(item) {
    return item.type === 'message' && item.role === 'user';
}
function normalizeCompactionInputMessage(item) {
    return {
        id: item.id,
        type: 'message',
        role: 'user',
        content: item.content.map((content) => {
            ...
            if (content.type === 'input_image') {
                ...
                throw new UserError('Compaction input_image item missing image_url or file_id.');
            }
            if (content.type === 'input_file') {
                if (typeof content.file_data === 'string' &&
                ...
                throw new UserError('Compaction input_file item missing file_data, file_url, or file_id.');
            }
            throw new UserError(`Unsupported compaction message content type: ${JSON.stringify(content)}`);
        }),
    };
}
...
function areAgentItemsEqual(left, right) {
