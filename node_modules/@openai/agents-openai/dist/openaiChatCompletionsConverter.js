...
exports.getCompatibleToolChoice = getCompatibleToolChoice;
...
function getCompatibleToolChoice(toolChoice, tools) {
    ...
    if (converted === 'required') {
        if (tools.length === 0) {
            throw new agents_core_1.UserError('modelSettings.toolChoice="required" requires at least one available tool in Chat Completions mode.');
        }
        return converted;
    }
    ...
    const availableToolNames = new Set(tools
        .map((tool) => tool.type === 'function' && 'function' in tool
        ? tool.function.name
        : undefined)
        .filter((name) => typeof name === 'string'));
    if (!availableToolNames.has(converted.function.name)) {
        throw new agents_core_1.UserError(`modelSettings.toolChoice="${converted.function.name}" does not match any available tool or handoff in Chat Completions mode.`);
    }
    return converted;
}
    ...
    for (const c of content) {
        if (c.type === 'output_text') {
            out.push({
                type: 'text',
                text: c.text,
                ...(0, providerData_1.getProviderDataWithoutReservedKeys)(c.providerData, ['type', 'text']),
            });
        }
            ...
            out.push({
                type: 'refusal',
                refusal: c.refusal,
                ...(0, providerData_1.getProviderDataWithoutReservedKeys)(c.providerData, [
                    'type',
                    'refusal',
                ]),
            });
...
function extractAllUserContent(content) {
    ...
    for (const c of content) {
        if (c.type === 'input_text') {
            ...
            const rest = (0, providerData_1.getProviderDataWithoutReservedKeys)(c.providerData, [
                'type',
                'image_url',
            ]);
            const imageUrl = (0, providerData_1.getProviderDataWithoutReservedKeys)(c.providerData?.image_url, ['url']);
            out.push({
                type: 'image_url',
                image_url: {
                    url: imageSource,
            ...
            if (typeof c.audio !== 'string') {
                throw new agents_core_1.UserError(`Chat Completions only supports inline audio data for input_audio: ${JSON.stringify(c)}`);
            }
            const inputAudioFormat = c.format === 'wav' || c.format === 'mp3'
                ? c.format
                : c.providerData?.input_audio?.format === 'wav' ||
                    ...
                    : undefined;
            if (!inputAudioFormat) {
                throw new agents_core_1.UserError(`Chat Completions input_audio requires format "wav" or "mp3": ${JSON.stringify(c)}`);
            }
            const rest = (0, providerData_1.getProviderDataWithoutReservedKeys)(c.providerData, [
                'type',
                'input_audio',
            ]);
            const inputAudio = (0, providerData_1.getProviderDataWithoutReservedKeys)(c.providerData?.input_audio, ['data', 'format']);
            out.push({
                type: 'input_audio',
                input_audio: {
                    data: c.audio,
...
function itemsToMessages(items, options = {}) {
        ...
        if (isMessageItem(item)) {
            ...
            if (role === 'assistant') {
                const assistant = {
                    ...
                    ...(0, providerData_1.getProviderDataWithoutReservedKeys)(providerData, [
                        'role',
                        'content',
                        'tool_calls',
                        'audio',
                ...
                toolCalls.push({
                    ...
                    function: {
                        ...
                        arguments: JSON.stringify({
                            ...
                            ...(0, providerData_1.getProviderDataWithoutReservedKeys)(argumentData, [
                                'queries',
                                'status',
                            ]),
                        }),
                        ...(0, providerData_1.getProviderDataWithoutReservedKeys)(remainingFunctionData, [
                            'name',
                            'arguments',
                        ]),
                    },
                    ...(0, providerData_1.getProviderDataWithoutReservedKeys)(rest, [
                        'id',
                        'type',
                        'function',
                    ]),
            ...
            throw new agents_core_1.UserError('Tool search items are not supported for chat completions. Please use the Responses API when replaying tool search history.');
        }
        else if (item.type === 'function_call') {
            const hasQualifiedOrInvalidName = typeof item.name === 'string' &&
                !CHAT_COMPLETIONS_FUNCTION_NAME_PATTERN.test(item.name);
            if (hasQualifiedOrInvalidName ||
                (typeof item.namespace === 'string' && item.namespace.trim().length > 0)) {
                throw new agents_core_1.UserError('Namespaced function call history is not supported for chat completions. Please use the Responses API when replaying namespaced tool calls.');
            }
            const asst = ensureAssistantMessage();
            const toolCalls = asst.tool_calls ?? [];
            const funcCall = item;
            const toolCallProviderData = (0, providerData_1.getProviderDataWithoutReservedKeys)(funcCall.providerData, ['id', 'type', 'function', 'role', 'content', 'tool_calls', 'audio']);
            const functionProviderData = (0, providerData_1.getProviderDataWithoutReservedKeys)(funcCall.providerData?.function, ['name', 'arguments']);
            toolCalls.push({
                id: funcCall.callId,
                type: 'function',
                function: {
            ...
            asst.tool_calls = toolCalls;
            Object.assign(asst, (0, providerData_1.getProviderDataWithoutReservedKeys)(funcCall.providerData, [
                'role',
                'content',
                'tool_calls',
                'audio',
            ...
            const toolContent = normalizeFunctionCallOutputForChat(funcOutput.output, options);
            result.push({
                role: 'tool',
                tool_call_id: funcOutput.callId,
                content: toolContent,
                ...(0, providerData_1.getProviderDataWithoutReservedKeys)(funcOutput.providerData, [
                    'role',
                    'tool_call_id',
                    'content',
                ]),
...
function normalizeFunctionCallOutputForChat(output, options) {
...
function handleEmptyOrNonTextToolOutput(options) {
    const message = 'Chat Completions tool outputs cannot be empty or contain only non-text content.';
    if (options.strictFeatureValidation) {
        throw new agents_core_1.UserError(message);
    }
    logger_1.default.warn(`${message} Replacing the tool output with a placeholder; enable strict feature validation to raise an error instead.`);
