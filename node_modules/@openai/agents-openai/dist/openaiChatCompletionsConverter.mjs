...
import { getProviderDataWithoutReservedKeys } from "./utils/providerData.mjs";
...
export function getCompatibleToolChoice(toolChoice, tools) {
    ...
    if (converted === 'required') {
        if (tools.length === 0) {
            throw new UserError('modelSettings.toolChoice="required" requires at least one available tool in Chat Completions mode.');
        }
        return converted;
    }
    ...
    if (!availableToolNames.has(converted.function.name)) {
        throw new UserError(`modelSettings.toolChoice="${converted.function.name}" does not match any available tool or handoff in Chat Completions mode.`);
    }
    return converted;
}
...
export function extractAllUserContent(content) {
    ...
    for (const c of content) {
        if (c.type === 'input_text') {
            ...
            const imageUrl = getProviderDataWithoutReservedKeys(c.providerData?.image_url, ['url']);
            out.push({
                type: 'image_url',
                image_url: {
                    url: imageSource,
            ...
            if (typeof c.file === 'string') {
                ...
                if (value.startsWith('data:')) {
                    ...
                    throw new UserError(`Chat Completions only supports data URLs for file input. If you're trying to pass an uploaded file's ID, use an object with the id property instead: ${JSON.stringify(c)}`);
                }
            }
                ...
                throw new UserError(`File input requires a data URL or file ID: ${JSON.stringify(c)}`);
            }
            ...
            if (typeof c.audio !== 'string') {
                throw new UserError(`Chat Completions only supports inline audio data for input_audio: ${JSON.stringify(c)}`);
            }
            const inputAudioFormat = c.format === 'wav' || c.format === 'mp3'
                ...
                : c.providerData?.input_audio?.format === 'wav' ||
                    ...
                    : undefined;
            if (!inputAudioFormat) {
                throw new UserError(`Chat Completions input_audio requires format "wav" or "mp3": ${JSON.stringify(c)}`);
            }
            const rest = getProviderDataWithoutReservedKeys(c.providerData, [
                'type',
                'input_audio',
            ]);
            const inputAudio = getProviderDataWithoutReservedKeys(c.providerData?.input_audio, ['data', 'format']);
            out.push({
                type: 'input_audio',
                input_audio: {
                    data: c.audio,
...
export function itemsToMessages(items, options = {}) {
    ...
    for (const item of items) {
        if (isMessageItem(item)) {
            ...
            throw new UserError('Tool search items are not supported for chat completions. Please use the Responses API when replaying tool search history.');
        }
            ...
            if (hasQualifiedOrInvalidName ||
                ...
                throw new UserError('Namespaced function call history is not supported for chat completions. Please use the Responses API when replaying namespaced tool calls.');
            }
            const asst = ensureAssistantMessage();
            const toolCalls = asst.tool_calls ?? [];
            const funcCall = item;
            const toolCallProviderData = getProviderDataWithoutReservedKeys(funcCall.providerData, ['id', 'type', 'function', 'role', 'content', 'tool_calls', 'audio']);
            const functionProviderData = getProviderDataWithoutReservedKeys(funcCall.providerData?.function, ['name', 'arguments']);
            toolCalls.push({
                id: funcCall.callId,
                type: 'function',
                function: {
            ...
            asst.tool_calls = toolCalls;
            Object.assign(asst, getProviderDataWithoutReservedKeys(funcCall.providerData, [
                'role',
                'content',
                'tool_calls',
                'audio',
            ...
            throw new UserError('Compaction items are not supported for chat completions. Please use the Responses API when working with compaction.');
        }
        else {
...
function normalizeFunctionCallOutputForChat(output, options) {
    ...
    if (Array.isArray(output)) {
        ...
        if (options.strictFeatureValidation && textItems.length !== output.length) {
            throw new UserError('Only text tool outputs are supported for chat completions. Got item: ' +
...
function handleEmptyOrNonTextToolOutput(options) {
    ...
    logger.warn(`${message} Replacing the tool output with a placeholder; enable strict feature validation to raise an error instead.`);
