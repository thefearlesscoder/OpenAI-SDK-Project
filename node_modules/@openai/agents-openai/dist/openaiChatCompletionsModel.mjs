import { Usage, UserError, withGenerationSpan, resetCurrentSpan, createGenerationSpan, setCurrentSpan, } from '@openai/agents-core';
...
import { getCompatibleToolChoice, toolToOpenAI, convertHandoffTool, itemsToMessages, } from "./openaiChatCompletionsConverter.mjs";
...
export class OpenAIChatCompletionsModel {
    ...
    async *getStreamedResponse(request) {
        ...
        try {
            ...
            for await (const event of convertChatCompletionsStreamToResponses(response, stream, { strictFeatureValidation: this.#strictFeatureValidation })) {
    ...
    #handleUnsupportedServerManagedConversationState(request) {
        ...
        if (!this.#hasWarnedUnsupportedConversationState) {
            logger.warn(`${message} Ignoring unsupported server-managed conversation state; ` +
    ...
    #handleUnsupportedPrompt(request) {
        ...
        if (!this.#hasWarnedUnsupportedPrompt) {
            logger.warn(`${message} Ignoring prompt; enable strict feature validation to raise an error instead.`);
    ...
    async #fetchResponse(request, span, stream) {
            ...
            for (const tool of request.tools) {
                if (tool.type === 'function') {
                    if (typeof tool.namespace === 'string' &&
                        ...
                        throw new UserError('Namespaced function tools created with toolNamespace() are only supported with the Responses API.');
                    }
                    if (tool.deferLoading === true) {
                        throw new UserError('Function tools with deferLoading: true are only supported with the Responses API.');
                    }
                }
        ...
        const requestData = {
            ...
            tool_choice: getCompatibleToolChoice(request.modelSettings.toolChoice, tools),
        ...
        const completion = await this.#client.chat.completions.create(requestData, requestOptions);
        if (logger.dontLogModelData) {
