...
async function* convertChatCompletionsStreamToResponses(response, stream, options = {}) {
    let usage = undefined;
    const state = {
        started: false,
        text_content: null,
    ...
    for await (const chunk of stream) {
        if (chunk.id && (response.id === openaiChatCompletionsModel_1.FAKE_ID || !response.id)) {
        ...
        yield {
            ...
            event: chunk,
            providerData: {
                rawModelEventSource: rawModelEvents_1.OPENAI_CHAT_COMPLETIONS_RAW_MODEL_EVENT_SOURCE,
            },
        };
        ...
        if (chunk.choices.length > 1 || unsupportedChoiceIndexes.length > 0) {
            ...
            if (!state.hasWarnedUnsupportedChoice) {
                logger_1.default.warn(`${message} Ignoring the other choices; enable strict feature validation to raise an error instead.`);
            ...
            for (const tc_delta of delta.tool_calls) {
                ...
                if (!(tc_delta.index in state.function_calls)) {
                    state.function_calls[tc_delta.index] = {
                        id: response.id || openaiChatCompletionsModel_1.FAKE_ID,
                        arguments: '',
                        name: '',
                        type: 'function_call',
                        callId: '',
    ...
    const outputs = [];
    const outputItemId = response.id || openaiChatCompletionsModel_1.FAKE_ID;
    if (state.reasoning) {
        outputs.push({
            type: 'reasoning',
            content: [],
...
//# sourceMappingURL=openaiChatCompletionsStreaming.js.map