...
const rawModelEvents_1 = require("./rawModelEvents.js");
...
function getContextManagement(contextManagement) {
    if (!contextManagement) {
        return undefined;
    }
    return contextManagement.map((entry) => (0, providerData_1.camelOrSnakeToSnakeCase)(entry));
}
...
function normalizeLegacyFileFromOutput(value) {
    const filename = typeof value.filename === 'string' && value.filename.length > 0
        ? value.filename
        : undefined;
    const referencedId = typeof value.id === 'string' && value.id.length > 0
        ? value.id
...
function getShellCallProviderDataForInput(providerData) {
    const normalized = (0, providerData_1.camelOrSnakeToSnakeCase)(providerData);
...
function toOpenAIShellSkill(skill) {
    if (skill.type === 'skill_reference') {
        const skillId = skill.skillId;
        if (typeof skillId !== 'string' || skillId.length === 0) {
            throw new agents_core_1.UserError('shell skill_reference requires skillId.');
        }
        return {
    ...
    if (skill.type === 'inline') {
        if (!skill.source) {
            throw new agents_core_1.UserError('shell inline skill requires a source.');
        }
        const mediaType = skill.source.mediaType;
        if (mediaType !== 'application/zip') {
            throw new agents_core_1.UserError('shell inline skill source.mediaType must be application/zip.');
        }
        if (typeof skill.source.data !== 'string' ||
            skill.source.data.length === 0) {
            throw new agents_core_1.UserError('shell inline skill source.data is required.');
        }
        if (typeof skill.name !== 'string' || skill.name.length === 0) {
            throw new agents_core_1.UserError('shell inline skill requires name.');
        }
        if (typeof skill.description !== 'string' ||
            skill.description.length === 0) {
            throw new agents_core_1.UserError('shell inline skill requires description.');
        }
        return {
            type: 'inline',
            name: skill.name,
    ...
    throw new agents_core_1.UserError(`Unsupported shell skill type: ${String(skill.type)}`);
}
function toOpenAIShellNetworkPolicy(policy) {
    if (!policy) {
        return undefined;
    }
    ...
    if (policy.type === 'allowlist') {
        if (!Array.isArray(policy.allowedDomains)) {
            throw new agents_core_1.UserError('shell allowlist networkPolicy requires allowedDomains.');
        }
        const allowedDomains = policy.allowedDomains.filter((domain) => typeof domain === 'string' && domain.length > 0);
    ...
    throw new agents_core_1.UserError(`Unsupported shell networkPolicy type: ${String(policy.type)}`);
}
function toOpenAIShellEnvironment(environment) {
    ...
    if (environment.type === 'local') {
        const localSkills = environment.skills?.map((skill) => {
            if (typeof skill.name !== 'string' ||
                ...
                throw new agents_core_1.UserError('Local shell skill requires name, description, and path.');
            }
            return {
                name: skill.name,
                description: skill.description,
    ...
    if (environment.type === 'container_reference') {
        ...
        if (typeof containerId !== 'string' || containerId.length === 0) {
            throw new agents_core_1.UserError('shell container_reference environment requires containerId.');
        }
        return {
...
function convertMCPRequireApproval(requireApproval) {
    const normalized = (0, utils_1.normalizeHostedMcpRequireApproval)(requireApproval);
    if (normalized === 'never') {
        return 'never';
    }
...
function getInputItems(input) {
    ...
    return input.map((item) => {
        ...
        if (item.type === 'shell_call') {
            ...
            const shellProviderData = getShellCallProviderDataForInput(item.providerData);
            const entry = {
                type: 'shell_call',
                id: item.id,
                call_id: item.callId,
        ...
        if (item.type === 'hosted_tool_call') {
            if (item.providerData?.type === 'web_search_call' ||
                ...
                const providerData = (0, providerData_1.camelOrSnakeToSnakeCase)(item.providerData) ?? {};
                const hasAction = providerData.action !== undefined;
                const hasValidAction = isRecord(providerData.action) &&
                    typeof providerData.action.type === 'string';
                if (hasAction && !hasValidAction) {
                    throw new agents_core_1.UserError('web_search_call invalid action');
                }
                const entry = {
...
function attachOpenAIResponseRequestId(response, requestId) {
    if (!requestId) {
        return;
    }
    const currentRequestId = getOpenAIResponseRequestId(response);
    if (currentRequestId) {
        return;
    }
    try {
...
async function* withAttachedResponseRequestId(stream, requestId) {
...
class OpenAIResponsesModel {
    ...
    async _fetchResponse(request, stream) {
        ...
        if (stream) {
            ...
            if (typeof withResponse === 'function') {
                ...
                response = withAttachedResponseRequestId(streamedResponse.data, streamedResponse.request_id ?? undefined);
            }
            else {
                response =
    ...
    _buildResponsesCreateRequest(request, stream) {
        ...
        let requestData = {
            ...
            context_management: getContextManagement(request.modelSettings.contextManagement),
            ...restOfProviderData,
        };
                ...
                if (eventType === 'response.created') {
                    ...
                    if (eventType === 'response.completed') {
                        yield {
                            ...
                            event: event,
                            providerData: {
                                rawModelEventSource: rawModelEvents_1.OPENAI_RESPONSES_RAW_MODEL_EVENT_SOURCE,
                            },
                        };
                    }
                }
...
function cloneResponsesWebSocketOptions(options) {
    return { ...(options ?? {}) };
}
/**
...
class OpenAIResponsesWSModel extends OpenAIResponsesModel {
    ...
    constructor(client, model, options = {}) {
        ...
        this.#websocketOptions = cloneResponsesWebSocketOptions(options.websocketOptions);
    }
    getRetryAdvice(args) {
    ...
    async #ensureWebSocketConnection(wsURL, headers, signal, requestTimeoutDeadline) {
        ...
        this.#wsConnection = await responsesWebSocketConnection_1.ResponsesWebSocketConnection.connect(wsURL, headers, signal, connectTimeout.timeoutMs, connectTimeout.errorMessage, this.#websocketOptions);
        this.#wsConnectionIdentity = identity;
